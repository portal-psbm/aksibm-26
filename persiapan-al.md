...
<style>
  ...
  .cabinet-container {
    ...
    box-shadow: inset 0 10px 20px rgba(0,0,0,0.3), 0 8px 24px rgba(0,0,0,0.2);
    position: relative;
  }
  ...
  .folder-tab {
    ...
    background: #d4c5a9; /* Warna manila folder */
  ...
  .al-content {
    ...
    position: relative;
    z-index: 5;
    margin-top: -1px; /* Menempel dengan folder aktif */
  }

  .al-panel { display: none; animation: fadeIn 0.4s ease; }
  .al-panel.active { display: block; }
  @keyframes fadeIn { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }

  /* ===== Komponen Internal (Card, Progress, dll) ===== */
  .card-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin: 16px 0; }
  .stat-card { background: #f8f9fa; border-radius: 10px; padding: 18px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); border-left: 4px solid #0d47a1; }
  ...
  @media (max-width: 768px) {
    .folder-shelf { overflow-x: auto; flex-wrap: nowrap; padding-bottom: 8px; -webkit-overflow-scrolling: touch; }
    .folder-tab { min-width: 110px; flex: 0 0 auto; }
